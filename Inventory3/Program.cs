...
namespace Inventory3
{
    class Program
    {
        ...
        {
            Console.WriteLine("{0} 입니다", x.Sum());
        }
        // 총입고 - 총출고를 계산하여 현재고를 보여줌
        static void ShowCurrentStock(int[] input, int[] output)
        {
            Console.WriteLine("** 현 재고량은 {0} 입니다", input.Sum() - output.Sum());
        }
        // 한달동안 일별 입출고 내역을 보여준다
        static void ShowMonthHistory(int[] input, int[] output)
        {
            int i;
            Console.WriteLine("날짜:입고:출고");
            for (i = 0; i < 31; i++)
            {
                Console.WriteLine("{0,4:D}:{1,4:D}:{2,4:D}", i + 1, input[i], output[i]);
            }
        }
